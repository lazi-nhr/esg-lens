...
async def evaluate(
    ...
    Args:
        ...
        criterion: ESG criterion (e.g., "emissions")
    ...
    retrieved_docs = await retrieve_similar(query, top_k)

    # Generate assessment
    assessment = await generate_answer(query, retrieved_docs)

    # Format the report
    if format == "markdown":
        report = format_report_markdown(company, criterion, query, retrieved_docs, assessment)
    else:
        report = format_report_text(company, criterion, query, retrieved_docs, assessment)
